...
import Actual from '../../data/aktual.json';
import _ from "lodash";
import queryString from "query-string";
import { Link, withRouter } from "react-router-dom";


class Aktual extends Component {
    handleClick = (ev) => {
        const queryObj = queryString.parse(window.location.search);
        queryObj.actual = ev?.target.id;
        ...
        return (
            ...
            {Actual.map((c) =>
                 <div onClick={this.handleClick} key={c.id}  class="category_check"> 
                    ...
                    <input required name="actual"  class="allcategory_checkbox" id={c.actual} value={c.actual} type="radio" placeholder="Поиск"/>  <label  for={c.actual}>  {_.upperFirst(c.actual)}</label>
...
export default withRouter(Aktual);